...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!ADMIN_ROLES.includes(role)) {
      setError('هذا الحساب مخصص لبوابة الموظف — استخدم الرابط أدناه')
  ...
  return (
    <div style={{
      ...
      padding: 20, position: 'relative', overflow: 'hidden', fontFamily: 'Cairo, sans-serif'
    }}>
      ...
      <div style={{ width: '100%', maxWidth: 400, position: 'relative' }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <div style={{ width: 64, height: 64, borderRadius: 18, margin: '0 auto 16px', background: 'linear-gradient(135deg,#2563eb,#60a5fa)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 8px 32px rgba(37,99,235,0.4)' }}>
            <Users size={30} color="#fff" />
          </div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: '#fff', margin: '0 0 4px' }}>نظام الموارد البشرية</h1>
          <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.45)', margin: 0 }}>لوحة تحكم الإدارة</p>
        </div>

        <div style={{ background: 'rgba(255,255,255,0.05)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, padding: 28 }}>
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <button type="submit" disabled={loading} style={{
              ...
              boxShadow: loading ? 'none' : '0 4px 16px rgba(37,99,235,0.4)', fontFamily: 'Cairo, sans-serif'
            }}>
            ...
            <div style={{ textAlign: 'center', paddingTop: 4, borderTop: '1px solid rgba(255,255,255,0.08)', marginTop: 4 }}>
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', margin: '0 0 8px' }}>هل أنت موظف؟</p>
              <a href="/employee/login" style={{ fontSize: 13, fontWeight: 600, color: '#60a5fa', textDecoration: 'none', padding: '8px 20px', borderRadius: 9, border: '1px solid rgba(96,165,250,0.3)', display: 'inline-block' }}>
                بوابة الموظف ←
              </a>
            </div>
          </form>
        ...
        <div style={{ textAlign: 'center', marginTop: 24 }}>
          <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)', margin: '0 0 6px' }}>© {new Date().getFullYear()} جميع الحقوق محفوظة</p>
