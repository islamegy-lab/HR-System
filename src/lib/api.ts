...
import type { Employee, Attendance, LeaveRequest, Payroll, DashboardStats } from '@/types'
...
export const recruitmentApi = {
  getJobs: async () =>
    supabase.from('recruitment_jobs')
      .select('*, department:departments(name,name_ar)')
      .order('created_at', { ascending: false }),
...
export const dashboardApi = {
  getStats: async (): Promise<{ data: DashboardStats | null; error: unknown }> => {
    ...
    const presentToday = todayAttendance.data?.filter(a => a.status === 'present').length || 0
    const absentToday = todayAttendance.data?.filter(a => a.status === 'absent').length || 0
    const onLeaveToday = todayAttendance.data?.filter(a => a.status === 'on_leave').length || 0
    const totalPayroll = monthlyPayroll.data?.reduce((sum, p) => sum + (p.net_salary || 0), 0) || 0
